...
public class ErrorHandlingBasics {
    public static void main(String[] args) {
        ...
        Observable.create(s -> {
            s.onError(new RuntimeException("failed1"));
        ...
        Observable.just(true).flatMap(v -> {
            if (v) {
                return Observable.error(new RuntimeException("failed4"));
            } else {
        ...
        Observable.error(new RuntimeException("failed5"))
        ...
        Observable.error(new IllegalStateException("failed6"))
                .onErrorResumeNext(t -> {
        ...
        Observable.error(new RuntimeException("failed7"))
                .onErrorResumeNext(t -> {
                    if (t instanceof IllegalStateException) {
                        return Observable.error(t);
                    } else {
